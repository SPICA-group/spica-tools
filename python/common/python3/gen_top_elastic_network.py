        ...
        if bbndx[0] == i:
            ...
            if resname[i] == "ALA":
                print ("atom %5d %5s %5s %5s %8.4f   %8.4f  P" \
                    %(I,resname[i],"ABT","ABT",mass,thischrg), file=ftop)
            else :
                print ("atom %5d %5s %5s %5s %8.4f   %8.4f  P" \
                    %(I,resname[i],"GBT","GBT",mass,thischrg), file=ftop)
            ...
            print ("atom %5d %5s %5s %5s %8.4f   %8.4f  P" \
                %(I,resname[i],name[i],wtype,mass,thischrg), file=ftop)
            ...
            if bBackbone[i1] == 1 and bBackbone[i2] == 1:
                ...
                if dr < MAXdr and resid[i2]-resid[i1] >= 3:
                    print ("bondparam %5d %5d   %f %f # %s-%s" \
                        %(I1,I2,kENM,dr,name[i1],name[i2]), file=ftop)
...
#######################################
    ...
    for i in range(nbb - 1):
        ...
        print ("bond %5d %5d # %s-%s" \
            %(bbndx[i]+1,bbndx[i+1]+1,name[bbndx[i]],name[bbndx[i+1]]), file=ftop)
                ...
                for j in spec_bond[bbtype[i]]:
                    ...
                    for k in j:
                        if bndx1 == 0:
                            ...
                            bndx2 = tmp_ndx
                    print ("bond %5d %5d # %s-%s" \
                        %(bndx1+1,bndx2+1,name[bndx1],name[bndx2]), file=ftop)
                    total_bonds+=1
        ...
        #CHECK TO SEE IF IT IS A SPECIAL DIHEDRAL
            if bbtype[i] in spec_dihed:
                ...
                print ("dihedralparam  %5d %5d %5d %5d   50.0 1 180 0.0 # %s-%s-%s-%s" \
                    %(ndx1+1,ndx2+1,ndx3+1,ndx4+1,name[bbndx[i]+1],name[bbndx[i]+2],name[bbndx[i]+3],name[bbndx[i]+4]), file=ftop)
