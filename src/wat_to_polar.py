...
def get_option_script(argv):
    argparser = ArgumentParser(usage='wat2polar input output',
                               prog ="wat2polar")
    argparser.add_argument('input', type=str,
                            help='Specify input CG PDB file name.')
    argparser.add_argument('output', type=str,
...
def write_pdb(f,pdb_list,l):
    ...
    if recname == "CRYST1":
        ...
        Lstr = pdb_list[l][7]
        Zval = pdb_list[l][8]
        print('%-6s%9.3f%9.3f%9.3f%7.2f%7.2f%7.2f %-11s%4s' %(recname,Lx,Ly,Lz,Aa,Ab,Ag,Lstr,Zval), file=f)
    elif recname == "ATOM":
...
class wat_to_polar:
    ...
    def run(self):
        ...
        angle_3 = [0.,0.,0.]
        pdb_list=[]
        nl = gen_line(self.infile)
        read_pdb(self.infile, pdb_list)
        ...
        for i in range(len(pdb_list)):
            if pdb_list[i][0].find("CRYST1") != -1:
                ...
                print(f"WARNING: {pdb_list[i][0]} cannot be recognized.")
        print(f"SPICA Water in '{self.infile}' has been successfully converted to pSPICA Water.")
        fo.close()
...
if __name__ == "__main__":
    ...
    infile = args.input
    outfile = args.output
    wat2polar = wat_to_polar(infile, outfile)
    wat2polar.run()
