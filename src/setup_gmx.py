            ...
            for jdx in range(idx, uniq_nats):
                ...
                if ifound == 0:
                    ...
                    if database.vdwstyle[vdwtmp] == "lj12_4":
                        ...
                        print("ERROR: WRITE CORRECT LJform {} {} (e.g.)lj12_4".format(database.vdwtype1[vdwtmp],database.vdwtype2[vdwtmp]))
                        sys.exit(0)
    ...
    with open("CGindex.ndx", "w") as fpind:
        for idx in range(sysdat.ntops):
            for jdx in range(topdat[idx].nmol):
                for kdx in range(topdat[idx].nat):
                    atindex += 1
                    if find_solv(topdat[idx].resname[kdx]) == 0:
                        if sol_flag == 0:
                            print("[ SOLVENT ]", file=fpind)
                            sol_flag = 1
        ...
        for idx in range(sysdat.ntops):
            for jdx in range(topdat[idx].nmol):
                for kdx in range(topdat[idx].nat):
                    atindex += 1
                    if find_prot(topdat[idx].resname[kdx]) == 0:
                        if pro_flag == 0:
                            print("[ PROTEIN ]", file=fpind)
                            pro_flag = 1
                        print("{:5d} ".format(atindex), end="", file=fpind)
                        indcnt += 1
                        if indcnt%15 == 0:
        ...
        for idx in range(sysdat.ntops):
            for jdx in range(topdat[idx].nmol):
                for kdx in range(topdat[idx].nat):
                    atindex += 1
                    if find_solv(topdat[idx].resname[kdx]) != 0 and find_prot(topdat[idx].resname[kdx]) !=0:
                        if oth_flag == 0:
                            print("[ OTHERS ]", file=fpind)
                            oth_flag = 1
                        print("{:5d} ".format(atindex), end="", file=fpind)
                        indcnt += 1
                        if indcnt%15 == 0:
        ...
        for idx in range(sysdat.ntops):
            for jdx in range(topdat[idx].nmol):
                for kdx in range(topdat[idx].nat):
                    atindex += 1
                    if find_solv(topdat[idx].resname[kdx]) != 0 and topdat[idx].charge[kdx] != 0:
                        if chr_flag == 0:
                            print("[ CHARGED ]", file=fpind)
                            chr_flag = 1
                        print("{:5d} ".format(atindex), end="", file=fpind)
                        indcnt += 1
                        if indcnt%15 == 0:
...
def find_solv(s):
    mol_list = ["WAT", "SOD", "CLA"]
    if s in mol_list:
        return 0
    return 1
def find_prot(s):
    mol_list = ["GLY", "ALA", "VAL", "LEU", "ILE",
                "SER", "THR", "MET", "CYS", "TYR",
   				"PHE", "TRP", "ASN", "GLN", "ASP",
   				"GLU", "ARG", "LYS", "PRO", "HIS",
   				"HSE", "HSD"] 
...
def make_top(database,topdat,sysdat):
    with open("topol.top", "w") as fout:
        print("; generated by setup_gromacs", file=fout)
        print("; RH DEVANE", file=fout)
        print("#include \"SPICA.itp\"", file=fout)
        print("#include \"molecule.itp\"", file=fout)
        print(file=fout)
        ...
        for idx in range(sysdat.ntops):
          print("{}     {}".format(topdat[idx].resname[0],topdat[idx].nmol), file=fout)
...
def run(args):
    ...
    for idx in range(ntops):
        ...
        read_top(inputs[2*idx], sysdat, topdat, idx)
        print("BOOKKEEPING:")
